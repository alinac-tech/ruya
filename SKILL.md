...
> AI ajanın senin gibi rüya görür. Sen uyurken hafızanı düzenler.
...
**Based on what you detect, set RUYA_MEMORY_TYPE:**

| Detection | RUYA_MEMORY_TYPE | Memory location | Session transcripts |
|-----------|-------------------|-----------------|-------------------|
...
Write the detected type to the config so ruya knows where to look:
```bash
echo "RUYA_MEMORY_TYPE=native" > ~/.claude/skills/ruya/.ruya-config
echo "RUYA_MEMORY_PATH=~/.claude/projects" >> ~/.claude/skills/ruya/.ruya-config
```

Replace `native` with `openclaw` or `project-root` if that's what was detected. For `openclaw` or `project-root`, also set the path:
```bash
# For openclaw:
echo "RUYA_MEMORY_TYPE=openclaw" > ~/.claude/skills/ruya/.ruya-config
echo "RUYA_MEMORY_PATH=$(pwd)/memory" >> ~/.claude/skills/ruya/.ruya-config

# For project-root:
echo "RUYA_MEMORY_TYPE=project-root" > ~/.claude/skills/ruya/.ruya-config
echo "RUYA_MEMORY_PATH=$(pwd)" >> ~/.claude/skills/ruya/.ruya-config
```

### Step 2: Install the Stop hook

Add the ruya trigger to your Claude Code settings:
...
if not any('ruya' in h.get('command','') for h in s['hooks']['Stop']):
    s['hooks']['Stop'].append({
        'type': 'command',
        'command': 'bash \$HOME/.claude/skills/ruya/should-ruya.sh && touch \$HOME/.claude/.ruya-pending || true'
    })
...
**What this does:** When you exit any Claude Code session, the Stop hook checks if 24+ hours have passed since the last ruya. If yes, it creates a `.ruya-pending` flag file. On your next session start, Claude sees the flag and runs `/ruya` as a background subagent.
...
```markdown
## Auto ruya
If the file `~/.claude/.ruya-pending` exists at session start, run `/ruya` as a subagent in the background, then delete the flag file: `rm ~/.claude/.ruya-pending`. This is the memory consolidation system - it runs automatically every 24 hours.
```

### Step 4: Make should-ruya.sh executable

```bash
chmod +x ~/.claude/skills/ruya/should-ruya.sh
```

### Step 5: Test it

Run a test ruya to verify the full flow:

```bash
# Force the ruya to trigger by creating the flag
touch ~/.claude/.ruya-pending
```

Then tell Claude: "The .ruya-pending flag exists. Run /ruya now."

After it completes, verify:
- The flag file `~/.claude/.ruya-pending` was deleted
- Memory files were reviewed and consolidated
- A `.last-ruya` timestamp was written in the memory directory
...
ruya runs in 4 sequential phases. Execute them in order. Do not skip phases.
...
```
Session ends
  --> Stop hook fires should-ruya.sh (~10ms)
  --> Checks: 24hrs passed? 5+ sessions?
  --> If NO: exits silently, zero overhead
  --> If YES: creates ~/.claude/.ruya-pending flag
Next session starts
  --> Claude reads CLAUDE.md, sees .ruya-pending exists
  --> Spawns /ruya as background subagent
  --> ruya runs all 4 phases
  --> Writes .last-ruya timestamp, deletes .ruya-pending
...
```bash
cat ~/.claude/skills/ruya/.ruya-config 2>/dev/null || echo "RUYA_MEMORY_TYPE=native"
```
...
### Record the ruya timestamp

After completing all 4 phases, write timestamps so the auto-trigger knows when you last dreamed:
```bash
date +%s > ~/.claude/projects/<project>/memory/.last-ruya
rm -f ~/.claude/.ruya-pending
```
